...
def do_diff(settings: Settings) -> None:
    ...
    try:
        old_packages = load_lockfile(api, settings.base_ref)
    except FileNotFoundError as e:
        print(f"Error loading base lockfile: {e}", file=sys.stderr)
        raise

    try:
        new_packages = load_lockfile(api, settings.ref)
    except FileNotFoundError as e:
        print(f"Error loading head lockfile: {e}", file=sys.stderr)
        raise

    package_diffs = diff(old_packages, new_packages)
    comment = format_comment(package_diffs)
